...
def docker_exec(cmdline):
    ...
    """
    local('docker exec -ti hmm_filter {}'.format(cmdline))
...
def inc_version():
    ...
    with open('hmm_filter/version.py', 'w') as f:
        f.write('__version__ = "{}.{}.{}"\n'.format(values[0], values[1], values[2]))
...
def git_check():
    ...
    output = local('git diff --name-only | egrep -v "^(hmm_filter/version.py)|(version.py)$" | tr "\\n" " "',
                   capture=True).strip()
    if output:
        fatal('Stage for commit and commit all changes first: {}'.format(output))

    output = local('git diff --cached --name-only | egrep -v "^(hmm_filter/version.py)|(version.py)$" | tr "\\n" " "',
                   capture=True).strip()
    if output:
...
def git_push():
    ...
    local('git add hmm_filter/version.py version.py')
...
def docker_build(options=''):
    ...
    Build docker image
    """
    local('docker build {} -t hmm_filter .'.format(options))
...
def docker_start(develop=True):
    ...
    local('docker run --rm --name hmm_filter -d -ti -p 127.0.0.1:8889:8888  -v {}:/code -t hmm_filter'.format(curr_dir))

    if develop:
        # Install package in develop mode: the code in /code is mapped to the installed package.
        docker_exec('python3 setup.py develop')

    print('Jupyter lab available at http://127.0.0.1:8889')
...
def docker_stop():
    ...
    """
    local('docker stop hmm_filter 2> /dev/null || true')
...
def test_pip(cleancontainer=True):
    ...
    docker_exec('pip install hmm_filter')
    test()
...
def release():
    ...
    build()
    pathname = 'dist/hmm_filter-{}.tar.gz'.format(version.__version__)
